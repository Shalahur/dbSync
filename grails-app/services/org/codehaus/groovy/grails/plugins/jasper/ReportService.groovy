...
class ReportService {
    ...
    private JasperPrint generatePrinter(JasperReportDef reportDef) {
        ...
        if (reportDef.reportData != null && !reportDef.reportData.isEmpty()) {
            ...
            Connection connection = DriverManager.getConnection(companyConfig.serverUrl+companyConfig.dbName, companyConfig.dbUser, companyConfig.dbPassword);

            //Sql sql = new Sql(dataSource)
            //Connection connection = dataSource?.getConnection()
            print "Datasource" + connection.getMetaData().getDriverName();
            try {
                if (resource.getFilename().endsWith('.jasper')) {
