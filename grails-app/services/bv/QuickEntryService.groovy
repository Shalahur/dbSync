...
import bv.quickentry.ReceiptEntry
...
//        }
   ...
   * */
    ...
    public LinkedHashMap listOfExpenseInvoice(String bookingPeriod, String fiscalYearId, String vendorId, String bookInvoiceId) {
       ...
       String expenseInvoice = """SELECT a.id as id, CONCAT(sp2.prefix,'-',a.invoice_no) AS invoiceNumber, a.booking_period AS bookingPeriod,a.vendor_id AS vendorId, DATE_FORMAT(a.trans_date,'%d-%m-%Y') AS invoiceDate, a.payment_ref AS paymentRef, a.total_gl_amount AS totalAmountIncVat, a.total_vat AS totalVat,CONCAT(iv.vendor_name,' [',sp.prefix,'-',iv.vendor_code,']') AS vendorName FROM invoice_expense AS a,budget_item_expense AS b, vendor_master AS v,vendor_master AS iv, system_prefix AS sp, system_prefix AS sp1, system_prefix AS sp2  WHERE a.budget_item_expense_id=b.id AND a.budget_vendor_id= v.id  AND a.vendor_id= iv.id  AND a.booking_year=${fiscalYearId}  AND a.budget_vendor_id= ${vendorId} AND sp.id=2 AND sp1.id=6 AND sp2.id=7 AND a.is_book_receive=0  """;
    ...
    public ReceiptEntry updateReceiptEntry(ReceiptEntry receiptEntry) {
        ...
        String deletePreviousReceiptDetailsQuery = """DELETE FROM receipt_entry_details WHERE receipt_entry_id=${receiptEntry.id}  """;
   ...
   * */
    ...
    public LinkedHashMap listOfManualReconcileEntrySearch(int offset, String limit, String sortItem, String sortOrder, String fiscalYearId, String searchItem, String searchString) {
        ...
        String countQuery = """SELECT  COUNT(a.id) AS totalManualRecList FROM Invoice_expense AS a, system_prefix AS sp WHERE  sp.id=7  AND STATUS = 1 AND a.paid_amount < (ROUND((a.total_gl_amount + a.total_vat),2)) ${wherePostCondition} ${searchBy} LIKE ${queryString}""";
        List<GroovyRowResult> count_result = db.rows(countQuery)
        int total = count_result[0].totalManualRecList
        db.close();
        return [manualEntryList: manualEntryList, count: total]
    ...
    public LinkedHashMap listOfUndoReconciliateItem(String selectedId ){

        def companyConfig = new BudgetViewDatabaseService().getConnectionInformation()
        def db = Sql.newInstance( companyConfig.serverUrl+companyConfig.dbName, companyConfig.dbUser, companyConfig.dbPassword, companyConfig.driverName)


        String transactionQuery = "SELECT a.id, date_format(b.trans_date_time,'%d %b %Y') AS date , b.by_bank_account_no, b.description,b.bank_payment_id,IF(b.debit_credit='D',CONCAT('-',b.amount) ,CONCAT('+',b.amount) )amount,b.debit_credit " +
                "FROM bank_statement_import_final AS a " +
                " INNER JOIN bank_statement_import_details_final AS b ON b.bank_import_id=a.id  " +
                " INNER JOIN trans_master AS t ON b.bank_payment_id=t.invoice_no  " +
                " WHERE a.trans_bank_account_no='${selectedId}' AND b.bank_payment_id IS NOT NULL  AND t.trans_type=7 GROUP BY b.bank_payment_id ORDER BY b.bank_payment_id "
        List<GroovyRowResult> undoReconciliateItemList = db.rows(transactionQuery);
