...
class ExtraSettingService {
    ...
    public LinkedHashMap listOfPaymentTerms(){
        ...
        String paymentTermQuery="""select a.id AS id,a.alert_repeat_days AS alertRepeatDays,a.alert_start_days AS alertStartDays,a.days_before_due AS daysBeforeDue, a.terms AS terms, a.status AS status, a.version As version from payment_terms AS a """
    ...
    public LinkedHashMap mainListOfFiscalYear(){
        ...
        String fiscalYearQuery="""select a.id AS id,date_format(a.year_begin,'%d-%m-%Y') AS yearBegin,date_format(a.year_end,'%d-%m-%Y') AS yearEnd,a.status AS status,a.version As version from fiscal_year AS a"""
    ...
    public LinkedHashMap listOfCompanyBankAccount(){
        ...
        String companyBankAccountQuery="""select a.id AS id,a.version As version,a.bank_account_code AS bankAccountCode,a.bank_account_name AS bankAccountName,a.bank_account_no AS bankAccountNo,a.bank_account_type AS bankAccountType,a.bank_address AS bankAddress,cb_gl.gl_chart_code AS companyBankGLCode,a.bank_name AS bankName,a.iban AS iban, a.status AS status from company_bank_accounts AS a,company_bank_gl_relation AS cb_gl where a.bank_account_code = cb_gl.bank_account_code """
...
//                ") as t group by fiscalYear;";
        ...
        String sqlExpenseIncome="SELECT YEAR(year_begin) as fiscalYear,'Income,Expense' as type1 FROM fiscal_year where YEAR(year_begin) <> '"+splitted[2]+"'  ";
    ...
    def writeOffInvoicePayment(def invoiceId,def paytmentReference,def transactionDate, def bookingPeriod, def paidAmount,def dueAmount, def bookingYear ){
        ...
        def date= "-12-31"
        def transdate=fiscalYearId.concat(date)
        println(""+invoiceId)
        def maxDBNum=0
        ...
        def recenciliationCode = invoiceId + "#2"
...
//                amount         : dueAmount,
//                companyBankCode: comapnyBankCode,
//                invoiceNo      : writeOffNextGenNum,
//                personCode     : 0,
...
//        new BudgetViewDatabaseService().insert(comBankTransRecon, tableNamecomBankTransRecon)
    ...
    def writeOffInvoicePaymentCustomer(def invoiceId,def paytmentReference,def transactionDate, def bookingPeriod, def paidAmount,def dueAmount, def bookingYear ){
