...
class DatabaseSynchService {
    ...
    static final DB_FILE_MAX_VERSION = 18
//    def currentDbName =""
//    def currentDbResult =""
    def finalReturn =""
//    def currentFile=""
//    def currentSql=""
    ...
    def updateAllDatabase(def index, def selectedDbName) {
        ...
        if(index){
            ...
            row = rows[Integer.parseInt(index)]
        }
        else if(selectedDbName){
            List<GroovyRowResult> rows = db.rows("""select db_name as dbName,db_user as dbUser,db_password as dbPassword, server_url as serverUrl from business_company WHERE db_name = ${selectedDbName}""");
            row = rows[0]
        }
...
//        currentDbName = dbName
    ...
    def parseDDL(def userDb, def dbVersion,def dbName) {
        ...
        String path = org.codehaus.groovy.grails.web.context.ServletContextHolder.getServletContext().getRealPath('sql')
        def bSuccess = false;
        //DDL parsing
    ...
    def resultMessage(def type,def ex,def dbName,def cFile,def cSql,def numberTable ){
        ...
        def finalReturn = ""
        if(type == "sqlError"){
            finalReturn = finalReturn+"<p style=\"color:red\">Database Name: "+ dbName+"<br>"
            finalReturn = finalReturn+"File Path: "+ cFile+"<br>"
            finalReturn = finalReturn+"Sql String: "+ cSql +"<br>"
            finalReturn = finalReturn+ dbName+" "+ "Is is failed. "+"<br>"
            finalReturn = finalReturn+"ERROR: "+ ex +"</p><br>***********************************************\n\n<br><br>"
        }
        else if(type == "corrupted"){
            finalReturn = finalReturn + "<p style=\"color:red\">Database Name: " + dbName + "<br>"
            finalReturn = finalReturn + "Tabel no:" + numberTable + "<br>"
            finalReturn = finalReturn + dbName + " " + "Is corrupted." + "<br>"
            finalReturn = finalReturn + "</p><br>***********************************************\n\n<br><br>"
        }
        else if(type == "notConnected"){
            finalReturn = finalReturn + "<p style=\"color:red\">Database Name: " + dbName + "<br>"
            finalReturn = finalReturn + dbName + " " + "failed to connect." + "<br>"
            finalReturn = finalReturn + "</p><br>***********************************************\n\n<br><br>"
        }
        else if(type == "successful")
        {
            finalReturn = finalReturn+"<p style=\"color:Green\">Database Name: "+ dbName+"<br>"
            finalReturn = finalReturn+"File Path: "+ cFile+"<br>"
            finalReturn = finalReturn+ dbName+" "+ "Is successfully executed. "+"</p><br> ***********************************************<br><br>"
        }
        else if(type == "ignore_statement")
        {
            finalReturn = finalReturn+"<p style=\"color:Green\">Database Name: "+ dbName+"<br>"
            finalReturn = finalReturn+ dbName+" "+ ex.getMessage()+"</p><br> ***********************************************<br><br>"
        }
    ...
    def List<Map> databaseMaintainceList() {
        ...
        for (def row in rows) {
            ...
            Map dbMaintainceMap = ["companyName"                : '', "databaseName": '', "creationDate": '', "firstLogin": '',
                                   "lastLogin"                  : '', "numberOfIncomeBudgets": '', "numberOfExpenseBudgets": '',
                                   "numberOfLinesInTrans_master": '', "status": ''];
            ...
            if (dbExist) {
                ...
                try {
                    ...
                    for(int i=0; i<resultExpenseInvoiceArr.size(); i++){
                        ...
                        def type=""
                        if(resultExpenseInvoiceArr[i][9]==0){type="Expense Invoice"}
                        else{type="Expense Receipt"}
...
//        finalReturn.databaseArr = dbVersionArr
//        finalReturn.countDB =countDB
...
//            finalResult = finalResult + "<div style=\"color:blue\">"+tempLine+"<br></div>"
    ...
    def dbDump(def tempDbName){
        def db = new Sql(dataSource);
        List<GroovyRowResult> rows = db.rows("""select db_name,db_user,db_password  from business_company where db_name= '${tempDbName}'""");
